...
int main() {
  ...
  int nbPoints =
      5; // IM PUTTING A SMALL VALUE HERE JUST FOR TESTING! (it was 40 before)
  ...
  for (int i = 0; i < nbPoints; i++) {
    //		cout<< i <<", X: "<<Xtarget[i]<<", Y: "<<Ytarget[i]<<", PolarR:
    //"<<PolarR[i]<< ", PolarTheta: "<<PolarTheta[i]<<endl;
  }
  vector<double> Zt;
  vector<double> Rt;
  vector<double> Xt;
  ...
  Xt_in.push_back(0);
  for (int k = 1; k < 5; k++) {
    ellipse2 = new Ellipse(0, 0, k * 0.25 * 0.2, k * 0.25 * 0.2, 0, nbPoints,
                           Xtarget, Ytarget, PolarR, PolarTheta);
  ...
  // Set of potential cross sections (Potential widths) // VALUE WAS ORIGINALLY
  // EQUAL TO 5!!!!!!!!!! <-------
  ...
  do {
    ...
    design = new Model2(IloEnv(), AMNS, CoilsR, CoilsZ, B_zero, tol, widths,
                        cross_sections, CCD, Gamma, zmin, zmax, rmin, rmax,
                        step, NbCoils);
    ...
    if (design->getCplexStatus() != IloAlgorithm::Infeasible &&
        ...
        for (int s = 0; s < Ns; s++) {
          if (SOL[n][s] * 1e7 > 1 || -SOL[n][s] * 1e7 > 1) {
            for (int m = 0; m < Nt; m++) {
              ...
              // field = new BFieldThick(J, CoilsR[n], widths[s],
              // widths[s]/Gamma, Xt[m], Zt[m], 0, CoilsZ[n], Bz, Br);
              B[m] = B[m] + Bz;
            }
          }
        }
        ...
        for (int n = 0; n < Nc; n++) {
          for (int s = 0; s < Ns; s++) {
            if (SOL[n][s] * 1e7 > 1 || -SOL[n][s] * 1e7 > 1) {
              ...
              field2 =
                  new BFieldThick(J, CoilsR[n], widths[s], widths[s] / Gamma,
      ...
      outFile << "Peak2Peak on the surface of the volums: " << Peak2Peak
              << endl;
