...
BField::BField(double i, double a, double z0, double r0, double zmin, double zmax, double rmin, double rmax, double step, 
...
#define MU0 (PI*4.0E-7)
	...
	for( double z = zmin-z0; z <= zmax-z0; in1++ ){
		...
		for(double r = rmin-r0; r <= rmax-r0; in2++){
			...
			Br[in1][in2] = (r==0.0)?(0.0):MU0 *(Ht*ga*(ek*(1+al2+be2)/(q-alt4)-fk));	
...
// BField::BField(double i, double a, double Zc, double Rc, double Zt, double Rt, double &Bz, double &Br)
// {
...
// 			std::cout<<"CoilsR[n]: "<<a<<", CoilsZ[n] : "<<Zc<<", Zt[m] : "<<Zt<<", Rt[m] : "<<Rt<<", AMN : "<< Bz<<", First: "<<fk<<", Second: "<< ek <<std::endl;
// 
// 
// 
// }
...
double B0 = (i*miu0)/(2*a);
		...
		{	
            ...
            //display(K);
            //display(E);
            Bz = B0*(1/(pi*sqrt(q)))*(E*(1 - al*al - be*be)/(q - 4*al) + K);    // axial component, e.g. along z-axis
            Br = B0*(ga/(pi*sqrt(q)))*(E*(1 + al*al + be*be)/(q - 4*al) - K);   // radial component
//        	std::cout<<"CoilsR[n]: "<<a<<", CoilsZ[n] : "<<Zc<<", Zt[m] : "<<Zt<<", Rt[m] : "<<Rt<<", AMN : "<< Bz<<", First: "<<K<<", Second: "<< E <<std::endl;

        }
        else    // if r = 0 which means that the magnetic field is computed along the main axis, e.g. z-axis, only
        { 
            Bz = B0*(a/sqrt(a*a + z*z))*(a/sqrt(a*a + z*z))*(a/sqrt(a*a + z*z));
            Br = 0;
        }
...
//     Br = (r==0.0)?(0.0):MU0*(Ht*ga*(ek*(1+al2+be2)/(q-alt4)-fk));
// 
// 	std::cout<<"CoilsR[n]: "<<a<<", CoilsZ[n] : "<<Zc<<", Zt[m] : "<<Zt<<", Rt[m] : "<<Rt<<", AMN : "<< Bz<<", First: "<<fk<<", Second: "<<ek <<", k :"<<k<<std::endl;
// 
//     /* display the output */
