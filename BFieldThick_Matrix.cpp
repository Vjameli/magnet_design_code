...
BFieldThick_Matrix::BFieldThick_Matrix(double J0, double r0, double dr,
                                       double dz, double xmin, double zmin,
                                       double xmax, double zmax, double xshift,
...
#define PI (3.14159265358979311599796346854418516)
  ...
  for (unsigned int in1 = 0; in1 < BzMatrix.size();
       in1++) { // defines the relative distance b/w coil and target point
    double x = xmin - xshift;
    for (unsigned int in2 = 0; in2 < BzMatrix[0].size();
         in2++) { // defines the relative lateral shift from z-axis b/w coil and
                  // target point
      double theta = 0;
      double z = 0;
      double r = 0;
      ...
      if (r >= 0.000001) {
        ...
        double dAdz = int1->N1(0.0, PI) - int2->N1(0.0, PI) -
                      int3->N1(0.0, PI) + int4->N1(0.0, PI);

        // cout << "Bfieldthick matrix dAdZ after" << endl;
        // dAdz = N1Integral(r0+dr/2,-dz/2,r,z) - N1Integral(r0-dr/2,-dz/2,r,z)
        // - N1Integral(r0+dr/2,dz/2,r,z) + N1Integral(r0-dr/2,dz/2,r,z); double
        // a1 = int3->N2(0.0, PI); double a2 = int4->N2(0.0, PI); double a3 =
        // int1->N2(0.0, PI); double a4 = int2->N2(0.0, PI);

        double dAdr = int3->N2(0.0, PI) - int4->N2(0.0, PI) -
                      int1->N2(0.0, PI) + int2->N2(0.0, PI);
        ...
        double term2 =
            ...
            (r0 - dr / 2 +
             sqrt((r0 - dr / 2) * (r0 - dr / 2) + (dz / 2 + z) * (dz / 2 + z)));
        BzMatrix[in1][in2] =
            C0 * PI * ((dz / 2 - z) * log(term1) + (dz / 2 + z) * log(term2));
      }
      x = x + s;
    }
    z1 = z1 + s;
...
void BFieldThick_Matrix::Polar(double x, double y, double z, double &r,
                               double &theta, double &z1) {
