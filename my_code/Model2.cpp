	...
	for (int s1 = 0; s1 < NbCS ; s1++)
		{
			 ...
			 {
	    		 if(abs(CoilsZ[n1] - CoilsZ[n2]) < widths[s1]/(Gamma*2) + widths[s2]/(2*Gamma) + L &&
	    			abs(CoilsR[n1] - CoilsR[n2]) < widths[s1]/2 + widths[s2]/2 + L)
	    			 {
	    			 IloExpr  exp1(env);
	    			 IloExpr  exp2(env);
	 ...
	 for (int s1 = 0; s1 < NbCS ; s1++)
		 {
	    	 for (int s2 = 0; s2 < NbCS ; s2++)
			 {
	    		 if(abs(CoilsZ[n1] - CoilsZ[n2]) < widths[s1]/(2*Gamma) + widths[s2]/(2*Gamma) + L &&
	    			 abs(CoilsR[n1] - CoilsR[n2]) < widths[s1]/2 + widths[s2]/2 + L)
	    			 {
	    			 IloExpr  exp1(env);
	    			 IloExpr  exp2(env);
...
void Model2::exportModel(char *file)
{
